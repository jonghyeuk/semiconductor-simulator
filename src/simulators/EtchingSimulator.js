...
import EtchingBasics from './EtchingBasics';
...
// ============================================================
// Si식각메커니즘 탭의 three.js 3D 씬은 제거했다.
//
// 가스 분해 → 라디칼 낙하 → 부산물 배출을 3D 로 그리던 1,540 줄짜리 컴포넌트였다.
// 장면은 그럴듯했지만 아무것도 재지 않았고, 그래서 "라디칼이 닿으면 식각된다" 까지만
// 말한 뒤 **왜 이온이 필요한지**는 끝내 설명하지 못했다. 실제로는 라디칼만으로는
// 거의 안 깎이고, 이온과 함께여야 빨라진다 (Coburn–Winters 1979).
//
// 그 자리에는 EtchingBasics(식각 기초)의 관측창을 embed 로 넣었다. 라디칼만·이온만·
// 동시를 각각 돌려 제거 속도를 직접 세고 시너지 배수를 산출한다.
// 반응 규칙과 확률은 src/physics/surfaceReaction.js, 문헌 상대비는 그 테스트가 지킨다.
// ============================================================
...
const EtchSimulator = ({ initialTab }) => {
  ...
  // 계산식은 src/physics/etching.js 로 옮겼다.

  // 슬라이더 변화에 따라 즉시 갱신되는 예상 결과 + 단면 프로파일 메트릭
  const liveResults = useMemo(() => {
    // 결정적 식각률: 난수원을 0.5 로 고정하면 배율이 정확히 1.0 이 되어
  ...
  const theorySteps = [
    ...
    },
    {
      ...
      content: "반도체 미세화와 함께 식각 기술도 비약적으로 발전했습니다.\n\n" +
               ...
               "📊 시장 규모: 글로벌 식각 장비 시장 약 200억 달러(2024, 조사기관별 편차 있음)\n" +
    ...
    switch (activeTab) {
                  ...
                  <div className="space-y-4">
                    <div className="bg-white p-4 rounded border-l-4 border-orange-400">
                      ...
                      <div className="text-sm space-y-2">
                        <p><strong>화학 반응식:</strong></p>
                        <div className="bg-gray-100 p-2 rounded font-mono text-center">
                          (C₇H₆O)ₙ + 8n O₂ → 7n CO₂ ↑ + 3n H₂O ↑   (PR 을 노볼락 반복단위 C₇H₆O 로 잡은 식)
                        </div>
                ...
                <div className="bg-green-50 p-5 rounded-lg border-l-4 border-green-400">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                    ...
                    <div className="bg-white p-4 rounded shadow">
                      ...
                      <ul className="space-y-1 text-gray-700">
                        <li>• <strong>2024년:</strong> 약 200억 달러 규모 (조사기관별 편차 있음)</li>
      ...
      case 'analysis':
        return (
          <div className="space-y-6">
            {/* 원자 스케일 표면 반응.
                예전에는 three.js 3D 씬이 가스 분해 → 라디칼 → 부산물을 보여 줬다.
                장면은 그럴듯했지만 아무것도 재지 않아서, "라디칼이 닿으면 식각된다" 까지만
                말하고 **왜 이온이 필요한지**는 끝내 설명하지 못했다. 실제로는 라디칼만으로는
                거의 안 깎인다. 이온과 함께여야 빨라진다.
                그 시너지를 직접 세어 보는 관측창으로 바꿨다. */}
            <EtchingBasics variant="embed" />
                ...
                <div className="bg-gradient-to-br from-blue-900 to-purple-900 p-4 rounded-lg border-2 border-blue-400">
                  ...
                  <div className="space-y-3 text-sm">
                    <div className="bg-black bg-opacity-30 p-3 rounded">
                      <p className="font-semibold text-yellow-300 mb-1">① 플라즈마 분해 (이 화면 위쪽에서)</p>
                      <p className="text-gray-300 text-xs">
                        관측창에 들어오는 라디칼은 이미 분해가 끝난 것들입니다. 분해 자체는
                        웨이퍼에서 한참 위, 플라즈마 벌크에서 일어납니다.
                      </p>
                      <div className="text-gray-300 text-xs mt-2 space-y-1">
                        <div>• <strong>SF₆</strong> → S + 6F* &nbsp; (황은 배출, 불소가 식각종)</div>
                        <div>• <strong>CF₄</strong> → C + 4F* &nbsp; (탄소는 폴리머로도 간다)</div>
                        <div>• <strong>Cl₂</strong> → 2Cl*</div>
                        <div>• <strong>HBr</strong> → H + Br*</div>
                      </div>
                    </div>

                    <div className="bg-black bg-opacity-30 p-3 rounded">
                      <p className="font-semibold text-green-300 mb-1">② 라디칼 흡착 (관측창 · 녹색 점)</p>
                      <p className="text-gray-300 text-xs">
                        라디칼은 <strong>방향이 제멋대로</strong>입니다. 비스듬히 들어와 표면 Si 에
                        달라붙고, Si–Si 결합을 Si–Cl 로 바꿔 놓습니다. 표면 원자가 초록빛으로
                        짙어지는 것이 염소화가 쌓이는 것입니다 (계기판의 「표면 염소화」).
                        <strong className="text-yellow-300"> 그런데 여기까지로는 거의 안 깎입니다.</strong>
                      </p>
                    </div>

                    <div className="bg-black bg-opacity-30 p-3 rounded">
                      <p className="font-semibold text-orange-300 mb-1">③ 이온 충격 (관측창 · 노란 사각)</p>
                      <p className="text-gray-300 text-xs">
                        이온은 시스 전기장에 가속돼 <strong>수직으로</strong> 내리꽂힙니다.
                        약해진 Si–Cl 결합을 때려 통째로 떼어냅니다. 깨끗한 Si 를 때리면 거의 못
                        떼어내고, 염소화된 Si 를 때리면 잘 떼어냅니다 — 이 차이가 시너지의 전부입니다.
                      </p>
                      <div className="text-gray-300 text-xs mt-2 space-y-1">
                        <div>• <strong>Cl₂ 계:</strong> Si + 4Cl* → SiCl₄↑</div>
                        <div>• <strong>불소계:</strong> Si + 4F* → SiF₄↑</div>
                        <div>• <strong>HBr 계:</strong> Si + 4Br* → SiBr₄↑</div>
                      </div>
                    </div>

                    <div className="bg-black bg-opacity-30 p-3 rounded">
                      <p className="font-semibold text-cyan-300 mb-1">④ 직접 재 보세요 (핵심!)</p>
                      <p className="text-gray-300 text-xs">
                        관측창의 <strong>라디칼만 / 이온만 / 동시</strong> 세 버튼을 각각 15 초씩
                        돌리면 제거 속도가 세어집니다. 동시 조건이 나머지 둘을 <strong>합친 것보다</strong>
                        훨씬 빠르다는 것 — 식각이 “화학 + 물리”가 아니라
                        <strong className="text-yellow-300"> 화학 × 물리</strong>인 이유입니다.
                        1979 년 Coburn–Winters 가 측정한 그 실험입니다.
                      </p>
                    </div>
                  </div>
                </div>
