...
const PlasmaSimulator = () => {
  ...
  const tabs = [
    { id: 'plasma-basics', name: '플라즈마 기본 특성', icon: '⚡' },
    { id: 'plasma-principle1', name: '플라즈마 발생원리1', icon: '🔬' },
    { id: 'plasma-principle2', name: '플라즈마 발생원리2', icon: '📈' },
    { id: 'rf-matching', name: 'RF 및 매칭', icon: '📡' },
    { id: 'system-structure', name: '시스템 구조(CCP)', icon: '🏗️' },
    { id: 'advanced-topics', name: '고급 주제', icon: '🎓' }
  ];
  ...
  const calculateOptimalLC = () => {
    ...
    if (Z_load > Z_source) {
      ...
      return {
        L: ((X_L / omega) * 1e9).toFixed(0),
        C: ((1 / (omega * X_C)) * 1e12).toFixed(0),
        type: 'L-type (직렬L-병렬C)'
      };
    } else {
    ...
    <div className="flex-1 flex flex-col">
      ...
      <div className="flex-1 overflow-auto">
        <div className="p-6">
          ...
          {activeTab === 'advanced-topics' && (
            <div className="space-y-8">
              <div className="bg-gradient-to-r from-indigo-50 to-blue-50 rounded-xl p-6 border">
                <h2 className="text-2xl font-bold text-indigo-900 mb-4">🎓 고급 주제</h2>
                <p className="text-indigo-700 mb-2">플라즈마 물리학의 심화 개념과 최신 연구 동향을 살펴봅니다.</p>
                <div className="text-sm text-indigo-600 bg-indigo-100 rounded-lg p-3">
                  <strong>포함 내용:</strong> 플라즈마 화학, ICP vs CCP 비교, 미래 기술 동향, 실무 적용 사례
                </div>
              </div>

              <div className="bg-white rounded-xl shadow-lg p-6 border">
                <h3 className="text-lg font-semibold text-indigo-800 mb-4">플라즈마 화학반응</h3>
                <div className="space-y-6">
                  <div className="bg-yellow-50 p-4 rounded-lg border-l-4 border-yellow-400">
                    <h4 className="font-semibold text-yellow-800 mb-2">식각 공정의 화학반응</h4>
                    <div className="space-y-3 text-yellow-700 text-sm">
                      <div><strong>실리콘 식각:</strong> Si + 4F → SiF₄ (기체) + 열</div>
                      <div><strong>산화막 식각:</strong> SiO₂ + 4CF₄ → SiF₄ + 2CO₂ + 4F</div>
                      <div><strong>폴리실리콘:</strong> Si + Cl₂ → SiCl₂ (기체)</div>
                    </div>
                  </div>
                  
                  <div className="bg-green-50 p-4 rounded-lg border-l-4 border-green-400">
                    <h4 className="font-semibold text-green-800 mb-2">증착 공정의 화학반응</h4>
                    <div className="space-y-3 text-green-700 text-sm">
                      <div><strong>질화막 증착:</strong> SiH₄ + NH₃ → Si₃N₄ + H₂</div>
                      <div><strong>산화막 증착:</strong> SiH₄ + N₂O → SiO₂ + N₂ + H₂</div>
                      <div><strong>다이아몬드 증착:</strong> CH₄ + H₂ → C(다이아몬드) + H₂</div>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white rounded-xl shadow-lg p-6 border">
                <h3 className="text-lg font-semibold text-indigo-800 mb-4">플라즈마 진단 기술</h3>
                <div className="grid md:grid-cols-2 gap-6">
                  <div className="bg-blue-50 p-4 rounded-lg">
                    <h4 className="font-semibold text-blue-800 mb-3">광학적 진단</h4>
                    <ul className="text-blue-700 text-sm space-y-1">
                      <li>• <strong>OES:</strong> 광방출 분광법으로 라디칼 밀도 측정</li>
                      <li>• <strong>FTIR:</strong> 적외선 분광으로 가스 농도 분석</li>
                      <li>• <strong>Laser Interferometry:</strong> 전자 밀도 측정</li>
                      <li>• <strong>LIF:</strong> 레이저 유도 형광으로 온도 측정</li>
                    </ul>
                  </div>
                  
                  <div className="bg-purple-50 p-4 rounded-lg">
                    <h4 className="font-semibold text-purple-800 mb-3">전기적 진단</h4>
                    <ul className="text-purple-700 text-sm space-y-1">
                      <li>• <strong>Langmuir Probe:</strong> 전자 온도/밀도 직접 측정</li>
                      <li>• <strong>RF Probe:</strong> 플라즈마 전위 측정</li>
                      <li>• <strong>Mass Spectrometry:</strong> 이온 에너지 분포</li>
                      <li>• <strong>Impedance Analysis:</strong> 플라즈마 임피던스</li>
                    </ul>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white rounded-xl shadow-lg p-6 border">
                <h3 className="text-lg font-semibold text-indigo-800 mb-4">미래 기술 동향</h3>
                <div className="space-y-6">
                  <div className="bg-gradient-to-r from-cyan-50 to-blue-50 p-4 rounded-lg">
                    <h4 className="font-semibold text-cyan-800 mb-3">원자층 공정 (ALE/ALD)</h4>
                    <p className="text-cyan-700 text-sm mb-2">
                      <strong>Atomic Layer Etching (ALE):</strong> 단일 원자층 단위로 정밀 제어하는 식각 기술
                    </p>
                    <ul className="text-cyan-600 text-sm space-y-1">
                      <li>• 3nm 이하 공정에서 필수 기술</li>
                      <li>• 표면 손상 최소화</li>
                      <li>• 선택비 100:1 이상 달성</li>
                    </ul>
                  </div>
                  
                  <div className="bg-gradient-to-r from-pink-50 to-purple-50 p-4 rounded-lg">
                    <h4 className="font-semibold text-pink-800 mb-3">대기압 플라즈마</h4>
                    <p className="text-pink-700 text-sm mb-2">
                      <strong>Atmospheric Plasma:</strong> 진공 없이 대기압에서 동작하는 플라즈마
                    </p>
              ...
              <div className="bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl p-6 border border-green-200">
                <h3 className="text-xl font-bold text-green-800 mb-4">🔬 실무에서의 플라즈마 엔지니어링</h3>
                
                <div className="space-y-6">
                  <div className="bg-white p-4 rounded-lg shadow-sm">
                    <h4 className="font-semibold text-green-800 mb-3">공정 개발 단계</h4>
                    <div className="grid md:grid-cols-4 gap-3">
                      <div className="text-center p-3 bg-green-50 rounded">
                        <div className="text-2xl mb-2">🎯</div>
                        <div className="font-semibold text-green-800">1. 요구사항</div>
                        <div className="text-xs text-green-600">목표 사양 정의</div>
                      </div>
                      <div className="text-center p-3 bg-green-50 rounded">
                        <div className="text-2xl mb-2">🧪</div>
                        <div className="font-semibold text-green-800">2. 실험 설계</div>
                        <div className="text-xs text-green-600">DOE, 변수 선정</div>
                      </div>
                      <div className="text-center p-3 bg-green-50 rounded">
                        <div className="text-2xl mb-2">📊</div>
                        <div className="font-semibold text-green-800">3. 데이터 분석</div>
                        <div className="text-xs text-green-600">통계적 최적화</div>
                      </div>
                      <div className="text-center p-3 bg-green-50 rounded">
                        <div className="text-2xl mb-2">🏭</div>
                        <div className="font-semibold text-green-800">4. 양산 적용</div>
                        <div className="text-xs text-green-600">스케일업, 모니터링</div>
                      </div>
                    </div>
                  </div>

                  <div className="bg-white p-4 rounded-lg shadow-sm">
                    <h4 className="font-semibold text-green-800 mb-3">플라즈마 공정 최적화 사례</h4>
                    <div className="space-y-3 text-green-700 text-sm">
                      <div><strong>문제상황:</strong> 실리콘 비아홀 식각에서 선택비 부족 (목표: 50:1, 현재: 20:1)</div>
                      <div><strong>해결방법:</strong> 
                        <ul className="ml-4 mt-1 space-y-1">
                          <li>• 가스 비율 조정: C₄F₈/SF₆/Ar = 40/30/30 sccm</li>
                          <li>• 온도 최적화: 척 온도 -20°C → 균일한 폴리머 형성</li>
                          <li>• 펄스 플라즈마: 식각/보호 사이클 분리</li>
                        </ul>
                      </div>
                      <div><strong>결과:</strong> 선택비 55:1 달성, 프로파일 수직도 89° → 90°</div>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-6 border border-blue-200">
                <h3 className="text-xl font-bold text-blue-800 mb-4">💡 플라즈마 기술의 미래</h3>
                
                <div className="grid md:grid-cols-2 gap-6">
                  <div className="bg-white p-4 rounded-lg">
                    <h4 className="font-semibold text-blue-800 mb-3">신소재 공정</h4>
                    <ul className="text-blue-700 text-sm space-y-2">
                      <li>• <strong>2D 소재:</strong> 그래핀, MoS₂ 선택적 식각</li>
                      <li>• <strong>양자점:</strong> 플라즈마 합성으로 크기 제어</li>
                      <li>• <strong>페로브스카이트:</strong> 태양전지용 박막 증착</li>
                      <li>• <strong>CNT:</strong> 탄소나노튜브 성장 및 정렬</li>
                    </ul>
                  </div>
                  
                  <div className="bg-white p-4 rounded-lg">
                    <h4 className="font-semibold text-blue-800 mb-3">AI 융합 기술</h4>
                    <ul className="text-blue-700 text-sm space-y-2">
                      <li>• <strong>실시간 제어:</strong> 머신러닝 기반 공정 최적화</li>
                      <li>• <strong>예측 진단:</strong> 장비 고장 사전 감지</li>
                      <li>• <strong>디지털 트윈:</strong> 가상 플라즈마 시뮬레이션</li>
                      <li>• <strong>자율 공정:</strong> 무인 팹 구현</li>
                    </ul>
                  </div>
                </div>
              </div>
...
export default PlasmaSimulator;="bg-gradient-to-r from-green-50 to-blue-50 p-4 rounded-lg"><h4 className="font-semibold text-green-800 mb-2">🔌 매칭 네트워크란?</h4><p className="text-green-700">파워서플라이에서 플라즈마로 에너지를 전달하는 <strong>중간다리 역할</strong>을 하는 장치로, <strong>매칭박스(Matching Box)</strong>라고도 부릅니다.</p></div>
                  <div className="grid md:grid-cols-2 gap-6">
