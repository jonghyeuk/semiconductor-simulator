...
> ⚠️ **계산값에 대한 안내**
>
> 이 시뮬레이터의 숫자는 **교육용 모델**이 낸 값이며 **실제 장비의 계측값과 다릅니다.**
> 공정 파라미터 사이의 지배 관계와 경향을 익히는 것이 목적이고, 실제 결과는
> 챔버 형상·가스 순도·웨이퍼 이력·계측 방식에 따라 크게 달라집니다.
> **실제 공정 조건 산출이나 장비 판단에 그대로 사용하지 마십시오.**
>
> 모듈별 문헌 대비 오차와 모델이 담지 않은 물리는
> [VERIFICATION.md](./VERIFICATION.md) 의 "정확도와 한계" 절에 정리해 두었습니다.
